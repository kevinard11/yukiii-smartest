                            ...
                            for child3 in child2.children:
                                if child3.type == 'name':
                                    ...
                                    for child4 in child2.children:
                                        ...
                                        nodes = [1]
                                        statement_count = [0]
                                        extract_operators_operands_from_tree(child4, operators, operands, nodes, edges, statement_count)
    ...
    for child in node.children:
        ...
        if child.type == "function_definition":
            ...
            if function_name and function_body:
                ...
                nodes = [1]
                statement_count = [0]
                extract_operators_operands_from_tree(child, operators, operands, nodes, edges, statement_count)
...
# Fungsi untuk mengekstrak operator & operan dari tree
def extract_operators_operands_from_tree(node, operators, operands, nodes, edges, statement_count):
    if node.type in ["binary_expression", "assignment_expression", "unary_op_expression", "logical_expression"]:
        # Operator ditemukan di dalam binary/unary expression
        if len(node.children) > 1:
            operator_node = node.children[1]  # Operator biasanya child ke-1
            # operators.add(operator_node.type)
            # operator_count[0] += 1
            if operator_node.type in operators:
                operators[operator_node.type] += 1
            else:
                operators[operator_node.type] = 1

    elif node.type in ["variable_name"]:
        # Variabel dianggap sebagai operan
        # operands.add(node.text.decode())
        # operand_count[0] += 1
        if node.text.decode() in operands:
            operands[node.text.decode()] += 1
        else:
            operands[node.text.decode()] = 1

    elif node.type in ["integer", "string", "float", "boolean"]:
        # Konstanta sebagai operan
        # operands.add(node.text.decode())
        # operand_count[0] += 1
        if node.text.decode() in operands:
            operands[node.text.decode()] += 1
    ...
    if node.type in ["if_statement", "for_statement", "while_statement", "switch_statement", "try_statement"]:
        edges[0] += 2  # Satu edge untuk true, satu untuk false
        nodes[0] += 1

    elif node.type in ['catch_statement']:
        nodes[0] += 1

    elif node.type in ["return_statement"]:
        edges[0] += 1  # Return menghubungkan ke akhir fungsi

    if node.type in executable_statement_types:
        statement_count[0] += 1

    # Rekursif untuk semua anak node
    for child in node.children:
        extract_operators_operands_from_tree(child, operators, operands, nodes, edges, statement_count)
