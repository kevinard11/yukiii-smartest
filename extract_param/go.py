...
def get_functions(head_node, scope):
    ...
    for node in head_node.children:
        ...
        edges = [1]
        statement_count = [0]
        extract_operators_operands_from_tree(node, operators, operands, nodes, edges, statement_count)
...
def extract_operators_operands_from_tree(node, operators, operands, nodes, edges, statement_count):
    if node.type in ["binary_expression", "assignment_statement", "unary_expression", "logical_expression", "short_var_declaration"]:
    ...
    elif node.type in ["int_literal", "float_literal", "string_literal", "rune_literal"]:
